...
<div class="form-group  col-xl-3" style="margin-top: 25px;">
    <button data-toggle="modal"  data-target="#category_item_recipe"  class="btn btn-outline-success btn-round">Done</button>
</div>

<!--<label for="category_items"><?php echo $this->lang->line('modifier-items') ?></label>
<select onchange="add_cat_modifiers_item()"  id="category_items_modifier" name="category_items_modifier" class="selectBox border-success bg-white form-control">
    <option><?php echo $this->lang->line('modifier-items') ?></option>
<?php
if (!empty($items_cat)) {
    foreach ($items_cat as $items) {
        ?>
...
<div class="modal animated lightSpeedIn text-xs-left" id="category_item_recipe" tabindex="-1" role="dialog" aria-labelledby="myModalLabel33" aria-hidden="true">
                    ...
                    <div class="card-body collapse in">
                        <div class="card-block">
                            <form method="post" id="add_items_modifiers" action="Modifier/save_item_modifier">
                                <input type="hidden" value="0" id="selected_cat_item_id" name="modifierItemId">
                                <table class = "table">
                                    <thead>
                                        <tr>

...
<div class="form-group  col-xl-3 ">
 ...
 <ul id="recipe_sub_cat_combo_items">
        ...
        source: function (request, response) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#recipe_sub_cat_combo_items').html('');
                    for (i = 0; i < count; i++) {
                        var id = parseInt(resp[i]);
                        var length_name = id.toString().length;
                        var total = (+length_name) + 1;
                        var recipe_name = resp[i].substr(total);
                        $('#recipe_sub_cat_combo_items').append('<li id="recipe_name_' + id + '"><a  class="text-primary" onclick="get_recipe_id(' + id + ')" >' + recipe_name + '</a></li>');
                    }
